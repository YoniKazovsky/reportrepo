...
def substitute(tree, name, replacement):
    if tree[0] == 'var':
        ...
        # Evaluate both sides after substitution to ensure full reduction
        return evaluate(('app', substitute(tree[1], name, replacement),
...
def main():
    ...
    if os.path.isfile(input_arg):
        ...
        # Otherwise, check the input or treat the input as a direct expression
        if isValidLambda(input_arg) == False:
            print("Error: Invalid lambda expression syntax. Only valid lambda expressions are allowed.")
            sys.exit(1)
        else:
